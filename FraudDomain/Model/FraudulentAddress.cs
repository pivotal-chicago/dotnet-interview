...
namespace FraudDomain.Model
{
    public class FraudulentAddress
    {
        ...
        public bool Match(string street, string streetnumber, string city, string state, string zip)
        {
            return ((Street.ToLower() == street.ToLower()) && (StreetNumber.ToLower() == streetnumber.ToLower()) &&
                    (City.ToLower() == city.ToLower()) && (State.ToLower() == state.ToLower()) &&
                    (ZIP.ToLower() == zip.ToLower()));
        }

        protected bool Equals(FraudulentAddress other)
        {
            return Id == other.Id && string.Equals(Street, other.Street) &&
                   string.Equals(StreetNumber, other.StreetNumber) && string.Equals(City, other.City) &&
                   string.Equals(State, other.State) && string.Equals(ZIP, other.ZIP) &&
                   string.Equals(CaseId, other.CaseId);
        }
